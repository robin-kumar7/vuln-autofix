...
# CRITICAL FIX: Validate JSON schema before processing
if ! jq -e '.matches' "$GRYPE_JSON" &>/dev/null; then
  echo "Error: Grype JSON missing .matches field - schema may have changed or version incompatible." >&2
  echo "  Expected Grype JSON schema with .matches array" >&2
  exit 1
fi
